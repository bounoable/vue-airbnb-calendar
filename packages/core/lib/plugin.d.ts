...
export default interface Plugin {
    install(context: RootContext, helpers: {
...
export declare type PluginFn = Plugin['install'];
export interface RootPlugin {
    on?: {
        [K in keyof HTMLElementEventMap]?: (el: HTMLElement, ev: HTMLElementEventMap[K], context: RootContext) => any;
    };
}
export interface CalendarItemPlugin {
    on?: {
        [K in keyof HTMLElementEventMap]?: (item: CalendarItem, el: HTMLElement, ev: HTMLElementEventMap[K], helpers: CalendarItemPluginHelpers, context: CalendarContext) => any;
    };
    /**
